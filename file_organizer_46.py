...
def organize_files(source_dir, target_root, mode="copy", progress_bar=None, status_label=None):
    ...
    for i, src_path in enumerate(all_files):
        ...
        if dedup_key in seen:
            # 重复文件：无论拷贝还是移动，都移动到 Duplicates（方案A）
            dest_path = get_unique_filename(duplicates_dir, filename)
            try:
                shutil.move(src_path, dest_path)   # 重复文件统一移动
...
def main():
    global root
    root = tk.Tk()
    root.title("文件分类整理工具 v4.6")
    root.geometry("1000x720")

    tk.Label(root, text="文件分类整理工具 v4.6", font=("微软雅黑", 18, "bold")).pack(pady=15)
    tk.Label(root, text="支持拷贝 / 移动模式 | 自动按年份分类（other除外）", fg="blue").pack(pady=5)
    ...
    tk.Radiobutton(mode_frame, text="移动（整理后源文件将被删除）", variable=mode_var, value="move", fg="#d32f2f").pack(side="left")
