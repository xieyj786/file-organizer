...
def organize_files(source_dir, target_root, mode="copy", other_exts=None, progress_bar=None, status_label=None):
    ...
    dedup_extensions = {'.txt', '.pdf', '.doc', '.docx', '.xls', '.xlsx', '.ppt', '.pptx'}
    ...
    for i, src_path in enumerate(all_files):
        ...
        if ext in dedup_extensions:
            ...
            if sha256 and sha256 in seen:
                ...
                # 大小相近 + 时间相同 进行额外检查
                for existing_sha, _ in list(seen.items()):
    ...
    with open(log_path, "w", encoding="utf-8") as f:
        ...
        f.write(f"网页文件 (.mht): {stats['web']} 个\n")
...
def main():
    global root
    root = tk.Tk()
    root.title("文件分类整理工具 v5.1")
    root.geometry("1050x780")

    tk.Label(root, text="文件分类整理工具 v5.1", font=("微软雅黑", 18, "bold")).pack(pady=15)
    tk.Label(root, text=".mht → 网页文件 | .ppt/.pptx 加入查重 | SHA-256 去重", fg="blue").pack(pady=5)
