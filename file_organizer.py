...
# ==================== 主整理函数 ====================
def organize_files(source_dir, target_root, mode="copy", other_exts=None, progress_bar=None, status_label=None):
    ...
    other_extensions = {ext.strip().lower() for ext in (other_exts or "").split(',') if ext.strip()}
    ...
    dedup_extensions = {'.txt', '.pdf', '.doc', '.docx', '.xls', '.xlsx', '.ppt', '.pptx', '.mht', '.htm'}
    ...
    for i, src_path in enumerate(all_files):
        ...
        if is_duplicate:
            ...
            try:
                ...
                duplicates.append(f"{src_path} → {dest_path} (重复)")
    ...
    while os.path.exists(log_path):
        log_path = os.path.join(target_root, f"file_organizer_log ({counter}).txt")
        counter += 1

    with open(log_path, "w", encoding="utf-8") as f:
        f.write("=== 文件分类整理日志 v5.1（简洁版） ===\n")
        f.write(f"运行时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"程序耗时: {runtime}\n\n")
        f.write(f"源目录: {source_dir}\n")
        f.write(f"目标根目录: {target_root}\n")
        f.write(f"操作模式: {mode_name}\n\n")
        f.write(f"文本文件: {stats['text']} 个\n")
        f.write(f"网页文件 (.mht + .htm): {stats['web']} 个\n")
        f.write(f"其他文件: {stats['other']} 个\n")
    ...
    messagebox.showinfo("完成", f"整理完成！\n"
                                f"本次日志: {os.path.basename(log_path)}\n"
                                f"网页文件: {stats['web']} 个\n"
...
def main():
    global root
    root = tk.Tk()
    root.title("文件分类整理工具 v5.1（简洁版）")
    root.geometry("1060x760")

    tk.Label(root, text="文件分类整理工具 v5.1（简洁版）", font=("微软雅黑", 18, "bold")).pack(pady=15)
    tk.Label(root, text=".mht + .htm → 网页文件 | 速度优化版", fg="blue").pack(pady=5)
    ...
    tk.Entry(frame, textvariable=source_var, width=100).grid(row=1, column=0, sticky="ew", pady=4)
    ...
    tk.Entry(frame, textvariable=target_var, width=100).grid(row=3, column=0, sticky="ew", pady=4)
    ...
    extra_ext_var = tk.StringVar(value="")
    tk.Entry(frame, textvariable=extra_ext_var, width=100).grid(row=6, column=0, sticky="ew", pady=4)

    progress = ttk.Progressbar(frame, orient="horizontal", length=880, mode="determinate")
    progress.grid(row=7, column=0, columnspan=2, pady=20, sticky="ew")

    status_label = tk.Label(frame, text="就绪 - 已简化去重逻辑，适合大目录", fg="#0066cc", font=("", 10))
